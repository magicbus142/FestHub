...
export function ImagesPreview() {
  ...
  return <Card className="h-full">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Button variant="outline" size="sm" onClick={() => currentOrganization && navigate(`/org/${currentOrganization.slug}/images`)}>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {recentImages.length > 0 ? <div className="grid grid-cols-2 gap-2">
                {recentImages.map(image => <div key={image.id} className="relative aspect-square rounded-lg overflow-hidden bg-muted cursor-pointer hover:opacity-80 transition-opacity" onClick={() => currentOrganization && navigate(`/org/${currentOrganization.slug}/images`)}>
              ...
              </div> : <div className="text-center py-8 border-2 border-dashed border-muted-foreground/25 rounded-lg">
                <Image className="h-8 w-8 mx-auto text-muted-foreground mb-2" />
                <p className="text-sm text-muted-foreground">
                  {t('చిత్రాలు లేవు', 'No images yet')}
                </p>
                <Button variant="ghost" size="sm" className="mt-2" onClick={() => currentOrganization && navigate(`/org/${currentOrganization.slug}/images`)}>
